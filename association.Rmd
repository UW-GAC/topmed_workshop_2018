...
```{r}
workshop.path <- "https://github.com/UW-GAC/topmed_workshop_2018/tree/master/data"
snpfile <- "snv.tsv.gz"
if (!file.exists(snpfile)) download.file(file.path(workshop.path, snpfile), snpfile)
...
```{r message=FALSE}
#library(devtools)
#devtools::install_github("UW-GAC/wgsaparsr", ref = "5.0.6")
library(wgsaparsr)
...
The *wgsaparsr::get_fields()* function lists all of the annotation field headers in a WGSA output file:

```{r}
# list all fields in an annotation file (transpose to make pretty): 
t(get_fields("snp.tsv.gz"))
```

Only a subset of these annotations may be necessary for a particular association test, and it is unweildy to work with all of them, so it is useful to process the WGSA output file to select fields of interest.
