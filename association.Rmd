...
if (!file.exists(snvfile)) download.file(file.path(workshop.path, snvfile), snvfile, extra="-L")

indelfile <- "data/indel.tsv.gz"
if (!file.exists(indelfile)) download.file(file.path(workshop.path, indelfile), indelfile, extra="-L")
```
...
```{r eval=FALSE}
devtools::install_github("UW-GAC/wgsaparsr", ref = "5.0.7", upgrade_dependencies = FALSE)
```
...
This set can be saved and used with the analysis pipeline for association testing.
```{r}
save(aggregates, file = "data/variants_by_gene.RData")
```
...
Now you can proceed to an assocation testing exercise. In a larger-scale analysis of TOPMed data, aggregation units could include SNP and indel variants falling within defined other aggregation units (such as user-defined range-based aggregation, rather than gene definitions), and would not be restricted to the variants also present in this chosen subset of the 1000 Genomes Project. An analyst might also choose to filter variants within each unit based on various annotations (examples include loss of function, conservation, deleteriousness scores, etc.).

You will use the aggregation units you defined in an R dataframe. Each row of the dataframe specifies a variant (chromosome, position, ref, alt) and the group identifier (group_id) assigned to it. Mutiple rows with different group identifiers can be specified to assign a variant to different groups (a variant can be assigned to mutiple genes).

```{r agg_unit}
workshop.path <- "https://github.com/UW-GAC/topmed_workshop_2018/raw/master/"
aggfile <- "data/variants_by_gene.RData"
download.file(file.path(workshop.path, aggfile), snvfile, extra="-L")
