...
It also requires that the AnnotatedDataFrame `sample.id` is in the same order as the samples genetic relatedness matrix.
```{r}
...
```{r}
j <- match(annot$subject_id, rownames(mod$model.matrix))
annot$residuals <- mod$resid.marginal[j]
```
...
Both methods of looking at the variance components indicate that study 1 has a much smaller residual variance than the others.
...
Studies may have used different measurement equipment, which could produce some of the observed differences in height by study.
