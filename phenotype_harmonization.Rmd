...
We will start with a presentation, which is available here:
[]()
...
In this exercise, we will be working with some randomized phenotype data from three studies.

## Import phentoype files into R

The first step is to read the files into R for processing.
Before we begin, we need to download the data from github so you have access to it.
...
You will then have to make decisions about how to handle these differences or outliers before running your association analysis.


## Fit a linear model for statistical comparisons
...
It is also the same package that we use for the association analyses, so this exercise provides a brief introduction to the package and some of the data structures that are required.
...
The `GENESIS` code to fit the linear model requires a `scan_id` column.
Typically the `scan_id` column is linked to a sample id, not a subject id, for genotype-phenotype analyses.
...
Becase it is an input to the linear model, we need to download the genetic relatedness matrix calculated for these subjects.
```{r}
grmfile <- "grm.RData"
...
We will use `height` as the outcome variable and adjust for `sex`, `age`, and harmonzation unit (`study`) as fixed effects.
...
# put the phenotyep data and the GRM in the same order
...
On the other hand, the heteroskedastic model has three variance components - one for each study.
...
Because the heterogeneous model is preferred, we will work with it for the future checks.
...
We can also visually inspect the residuals by harmonization unit.
This is a visual representation similar to the likelihood ratio test we did earlier.
```{r}
ggplot(phen, aes(x = study, y = residuals)) +
  geom_violin() +
  geom_boxplot(width = 0.1)
```
By eye, `study_1` has a different variance than the others.

We also want to check if the different studies have different means.
This information is available in the fixed effects part of the null model, which we looked at earlier.
```{r}
nullmod_het$fixef
```
In particular, the rows whose names start with `study` have significant estimates associated with them.

We can also run a Wald test to see if the any one mean is significantly different from the average.
First, identify the fixed effects and covariance for the study indicator variables.
Then, calculate the Wald test statistic and associated p-value.
```{r}
idx <- 4:5
...
Very small values indicate that means are significantly different for at least one study.

## Final considerations

We have determined that the different studies have both different variance and different mean for height.
Before performing genotype-phenotype association tests with these data, you should think carefully about whether the phenotype is homogeneous enough to be analyzed together.
...
* a systematic measurement error in one study that leads to 
...
Unfortunately there is no single set of guidelines you can use to make this decision, so it requires careful consieration before proceeding with analyses.
